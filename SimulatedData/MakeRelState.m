...
%
function [sensor_expressedIn_prevSensor, E_sensor_expressedIn_prevSensor] = MakeRelState(sensor_expressedIn_world)

    sensor_expressedIn_prevSensor = zeros(size(sensor_expressedIn_world,1)-1, 6);
    
    %------------------My Change---------------------------------
    E_sensor_expressedIn_prevSensor = zeros(size(sensor_expressedIn_world,1)-1, 6);
    %------------------My Change---------------------------------
    ...
    for i = 2:size(sensor_expressedIn_world,1)
        ...
        %------------------My Change---------------------------------
        [sensor_expressedIn_prevSensor(i-1,:), E_sensor_expressedIn_prevSensor(i-1,:)] = GetState(inv(prevPose) * thisPose);
        %------------------My Change---------------------------------
        prevPose = thisPose;
    end